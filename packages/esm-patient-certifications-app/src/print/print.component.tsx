...
import { getCertificateConfig } from './registry';
...
export function PrintComponent({ subheader, patientDetails, encounter }: PrintComponentProps) {
  const { t } = useTranslation();
  const { logo } = useConfig();
  const session = useSession();

  // Load template config based on subheader and/or encounter details
  const config = getCertificateConfig(subheader, encounter);
  const TemplateComponent = config.templateComponent;

  // Render the dynamic template
  const renderTemplate = () => {
    if (!TemplateComponent) return null;
    return <TemplateComponent patientDetails={patientDetails} encounter={encounter} />;
  };

  return (
    <div className={styles.printPage}>
      {/* ── HEADER ── */}
      {config.showCommonHeader && (
        <header className={styles.header}>
          <div className={styles.headerTop}>
            <div className={styles.logoWrap}>
              {logo?.src ? (
                <img className={styles.logo} src={logo.src} alt={logo.alt ?? 'Logo'} />
              ) : logo?.name ? (
            ...
            <div className={styles.printedByWrap}>
              <span className={styles.printedByLabel}>{t('printedBy', 'Printed by')}:&nbsp;</span>
              <span className={styles.printedByValue}>{session?.user?.display}</span>
              <span className={styles.printedByLabel}>&nbsp;{t('onDate', 'on')}&nbsp;</span>
              <span className={styles.printedByValue}>{formatDate(new Date(), { noToday: true })}</span>
            </div>
          </div>

          {/* ── PATIENT INFO BAND ── */}
          {config.showCommonPatientBand && (
            <div className={styles.patientBand}>
              <div className={styles.patientField}>
                <span className={styles.fieldLabel}>{t('patient', 'Patient')}</span>
                <span className={styles.fieldValue}>{patientDetails?.name}</span>
              </div>
              <div className={styles.patientField}>
                <span className={styles.fieldLabel}>{t('gender', 'Gender')}</span>
                <span className={styles.fieldValue}>{patientDetails?.gender}</span>
              </div>
              <div className={styles.patientField}>
                <span className={styles.fieldLabel}>{t('age', 'Age')}</span>
                <span className={styles.fieldValue}>{patientDetails?.age}</span>
              </div>
              {patientDetails?.identifiers?.length > 0 && (
                <div className={styles.patientField}>
                  <span className={styles.fieldLabel}>{t('id', 'ID')}</span>
      ...
      {/* ── DOCUMENT TITLE BAND ── */}
      {config.showCommonHeader && (
        <div className={styles.subheaderBand}>
          <h2 className={styles.subheaderTitle}>{subheader}</h2>
        </div>
      )}
