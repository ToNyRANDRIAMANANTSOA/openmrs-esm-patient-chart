...
import { type Provider, type Location } from '../../print-selected-orders/types/prescription';
import { type ConfigSchema, type HeaderAlignment } from '../../config-schema';
...
const PrintHeader: React.FC<PrintHeaderProps> = ({
  ...
  const logoEl = logo?.src ? (
    ...
    <svg role="img" width={110} height={40} viewBox="0 0 380 119">
      <path
        fillRule="evenodd"
        clipRule="evenodd"
  ...
  return (
    ...
    >
      ...
      <div className={styles.printHeaderColumns}>
        {/* Column A: title (when not in shared row) + providerInfo */}
        <div className={`${styles.printSideA} ${alignClass(sideAAlignment)}`}>
          {!titleAndLogoInSharedRow && titleEl}
            ...
            <div className={styles.providerInfo}>
              {isLoadingProviders ? (
                ...
                <div>
                  ...
                  {provider?.attributes?.phoneNumber && (
                    <>
                      <br />
                      <strong>{t('phone', 'Phone:')}</strong> <span>{provider?.attributes?.phoneNumber}</span>
                    </>
                  )}
                </div>
              )}
        ...
        {/* Column B: logo (when not in shared row) + clinicInfo + facilityAddress */}
        <div className={`${styles.printSideB} ${alignClass(sideBAlignment)}`}>
          {!titleAndLogoInSharedRow && logoEl}

          {showClinicInfo && !isLoadingEncounters && (
            <strong className={styles.clinicInfo}>{location?.attributes?.display ?? location?.name}</strong>
          )}
          ...
          {showFacilityAddress && (
            <div className={styles.facilityAddress}>
              {isLoadingEncounters ? (
                ...
                <>
                  <strong>{t('contact', 'Contact:')}</strong> {location?.attributes?.phoneNumber1}
                  {location?.attributes?.phoneNumber2 && <> {location?.attributes?.phoneNumber2}</>}
                  <br />
                  <strong>{t('email', 'Email:')}</strong> {location?.attributes?.email}
                  <br />
                  {location?.attributes?.addressDisplay}
                </>
              )}
            </div>
          )}
