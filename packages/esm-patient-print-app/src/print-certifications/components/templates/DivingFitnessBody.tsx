...
const DivingFitnessBody: React.FC<CertificateBodyProps> = ({ certificate, isLoadingEncounters }) => {
  ...
  const divingType = getObsByConceptKeywords(obs, 'diving type', 'type de plongée');
  const maxDepth = getObsByConceptKeywords(obs, 'max depth', 'depth', 'profondeur');
      ...
      */}
      <p className={styles.bodyLong01}>
        ...
        {divingType && (
          <>
            {' '}
            (<strong>{divingType}</strong>)
          </>
        )}
        {maxDepth && (
          <>
            {', '}
            {t('certBody.diving.upToDepth', 'up to a maximum depth of')}{' '}
            <strong>
              {maxDepth} {t('certBody.diving.meters', 'metre(s)')}
            </strong>
          </>
        )}
        .
