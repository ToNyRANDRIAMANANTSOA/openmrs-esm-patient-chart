...
const NonContagionTemplate: React.FC<Props> = ({ patientDetails, encounter }) => {
  ...
  const disease = getObsByConceptKeywords(obs, 'maladie', 'pathologie', 'disease', 'affection');
  ...
  return (
    <div className={styles.certFullPage}>
      ...
      <h1 className={styles.certTitle}>{t('nonContagionCertTitle')}</h1>
      ...
      <div className={styles.certBody}>
        ...
        <div className={styles.certStatement}>
          {t('nonContagionStatement')}
          {disease ? (
            <>
              {' '}
              ({t('nonContagionDisease')} <strong>{disease}</strong>)
            </>
          ) : (
            ''
          )}{' '}
        ...
        <div className={styles.closingStatement}>{t('nonContagionCertClosing')}</div>
...
export default NonContagionTemplate;
